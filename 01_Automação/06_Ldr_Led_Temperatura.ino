...
"<body>" \
  ...
  "<p>" \
    ...
    "<span class='dht-labels'>Estatus do Led</span>" \
        ...
        "<button type='submit' class='button' id='teste'>ON%-.-%</button>" \
        "</form>" \
...
"setInterval(function ( ) {" \
  ...
  "xhttp.onreadystatechange = function() {" \
    ...
    "}" \
  "};" \
  "xhttp.open('GET', '/acionamento', true);" \
  "xhttp.send();" \
"}, 1000 ) ;" \
...
void handleLEDon() {
   ...
   digitalWrite(led1Pin,LOW); //LED is connected in reverse
